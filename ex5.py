...
def get_name(**kwargs):
    name = kwargs.get("name", "indéterminé")
    print(f"Nom : {name}")
    return name


def get_life_span(**kwargs):
    life_span_str = kwargs.get("life_span", "indéterminé")
    print(f"Espérance de vie : {life_span_str}")
    return life_span_str


esperances = []
for cat in liste:
    for attr in {"name", "life_span"} - cat.keys():
        cat[attr] = "indéterminé"
    get_name(**cat)
    span = get_life_span(**cat)
    if span != "indéterminé":
        try:
            min_max = list(map(int, span.split(" - ")))
            moyenne = sum(min_max) / len(min_max)
            esperances.append(moyenne)
        except Exception:
            continue

moyenne_esp = statistics.mean(esperances) if esperances else 0


attributs_uniques = set()
for cat in liste:
    attributs_uniques.update(cat.keys())
