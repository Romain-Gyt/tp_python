...
with open("recipes.txt", "w", encoding="utf-8") as f:
    for i, url in enumerate(urls):
        try:
            print(f"🔍 Scraping recette {i + 1} : {url}")
...
with open("recipes.json", "w", encoding="utf-8") as f_json:
    json.dump(recipes_data, f_json, indent=2, ensure_ascii=False)

print(f" {len(recipes_data)} recettes sauvegardées dans recipes.txt et recipes.json")